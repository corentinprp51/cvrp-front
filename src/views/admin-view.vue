    ...
    <div class="flex grid grid-cols-2 mt-[20px]">
      <div class="flex flex-col items-center">
        ...
        <ul v-if="!isPreloadingUsers" class="mt-[10px] w-[60%] shadow rounded-[8px]">
          <li v-for="(user, index) in users" :class="index % 2 === 0 ? 'bg-[#EFEFEF]' : 'bg-white'" class="grid grid-cols-[1fr_2fr_1fr_1fr_1fr_1fr] items-center rounded-[8px] py-[15px] px-[8px] text-[18px] text-start">
            <span>{{ user.id }}</span>
            <span>{{ user.username }}</span>
            <span class="font-bold">{{ user.isAdmin ? 'admin' : 'member' }}</span>
            <span class="justify-self-end"><user class="w-[25px] h-[25px] cursor-pointer" @click="router.push({name: 'admin-view-user', params:{id: user.id}})" /></span>
